...
\vspace{6pt}
\noindent \textit{Check first whether the build tools are already installed.} If the Pi has been used for Pico development before, these packages may already be present. To check, run:

\begin{verbatim}
cmake --version && arm-none-eabi-gcc --version && git --version
\end{verbatim}

\noindent If all three commands print a version number, the build tools are installed and you can skip to the next subsection. If any command reports ``command not found,'' run the install command below.
...
\begin{itemize}
    \item \texttt{apt install} downloads and installs the listed packages. Any packages already installed will be skipped automatically.
...
\vspace{6pt}
\noindent \textbf{Note:} The \texttt{cp} command above uses a relative path (\texttt{build/pio\_quadrature\_encoder.uf2}) and will only work if you run it from the project root folder \texttt{\textasciitilde/pico/pico-projects/Pico-Quad-Encoder}. If you are in a different folder, either \texttt{cd} back to the project root first, or supply the full path to the \texttt{.uf2} file.
